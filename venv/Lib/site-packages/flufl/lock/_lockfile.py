...
ERRORS: Final = (errno.ENOENT, errno.ESTALE)
...
class Lock:
    ...
    def refresh(
        ...
        *,
        ...
        if not self.is_locked and not unconditionally:
            ...
            msg = f'{self!r}: {self._read()}'
            raise NotLockedError(msg)
    ...
    def lock(self, timeout: Interval | None = None) -> None:
        ...
        #
        # This next call can fail with an EPERM.  I have no idea why, but I'm nervous about ignoring
        # it.  It seems to be a very rare occurrence, only happens from cron, and has only(?) been
        ...
        while True:
            ...
            try:
                ...
                if error.errno in ERRORS:
                    # 2025-05-08(warsaw): Is the following comment still relevant?
                    #
                    # In some Linux environments, it is possible to get an ENOENT, which is truly
                    # strange, because this means that self._claimfile didn't exist at the time of
                    # the os.link(), but self._write() is supposed to guarantee that this happens!
                    # I don't honestly know why this happens -- possibly due to weird caching file
                    # systems?  For now we just say we didn't acquire the lock and try again next
                    # time.
                    pass
                    ...
                    # Somebody's messin' with us!  Log this, and try again later.  Should we raise
                    # an exception?
                    log.error('unexpected linkcount: %s', self._linkcount)
                elif self._read() == self._claimfile:
                    # It was us that already had the link.
                    log.debug('already locked: %s', self._lockfile)
                    raise AlreadyLockedError('We already had the lock') from None
                # Otherwise, someone else has the lock.  The explicit pass below isn't needed but
    ...
    def _touch(self, filename: str | None = None) -> None:
    ...
    @property
    def _releasetime(self) -> int | datetime:
    ...
    def _break(self) -> None:
        """Break the lock."""
        # Try to read from the lock file.  All we care about is that its contents have the details
        # expected of any lock file.  If not, then this probably isn't a lock that needs breaking,
        # it's a Lock with a lock file pointing to an existing, unrelated file.  Refuse to break
        # that lock.  All we really need to do is to log and return.  If a timeout was given,
        # eventually the .lock() call will timeout.  However if no timeout was given, the .lock()
        # will block forever.
        #
        # Discard the return value but keep the linter happy.
        try:
            _ = self.details
        except NotLockedError:
            log.error("lockfile exists but isn't safe to break: %s", self._lockfile)
            return
